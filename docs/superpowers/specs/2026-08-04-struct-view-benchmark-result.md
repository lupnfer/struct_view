...
### Caveat: array fields (added 2026-08-05, array-support feature)

The benchmark recipe (`alarm_line`) predates array-field support and contains **no scalar/struct arrays**. Route A's inlining advantage does **not** extend to array fields: per the array-support spec §5, Route A does not inline array iteration — struct arrays go through `ArrayStructBlockProviderA` (a `ValueProvider` slotted into `FieldBinding`), so the array's per-element loop is reached via one virtual `provider->get()` call, identical in dispatch cost to Route B's `ArrayStructBlockProvider`. Scalar arrays are the same `LambdaProvider` in both routes. Consequence: **for recipes that include array fields, Route A's already-small (~2%) win shrinks further** — the array segments gain nothing from Route A's four-kind inlining. The decision to keep Route B as default is therefore reinforced for array-heavy recipes. If a future recipe is array-heavy AND hot-path-critical, re-run `bench_routes` with that recipe and revisit; the expectation is parity or near-parity on array segments.
